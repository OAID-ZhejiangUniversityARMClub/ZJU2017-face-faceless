...
class cosine_distance_verifier: public face_verifier
{
	public:
		...
		int search(float * f, std::vector<std::pair<int, float>>& ret);
		...
		cosine_distance_verifier(void){feature_len_=128;};

               ~cosine_distance_verifier(void);
		int get_db_size(){return (int)feature_db_.size();};
...
int cosine_distance_verifier::search(float * f, std::vector<std::pair<int, float>>& ret)
{

	//p_score[0]=0;
	//std::cerr << "feature_db_.size: " << feature_db_.size() << std::endl;
	...
	for(int i=0;i<feature_db_.size();i++)
	{
		...
		ret.push_back(std::pair<int, float>(e.face_id, score));
		/*if(score>p_score[0])
		{
			p_score[0]=score;
    ...
    //if(p_score[0]>0.55 && p_score[0]<0.81)   p_score[0]=0.81;
